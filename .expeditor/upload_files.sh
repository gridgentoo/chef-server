...
aws s3 cp chef-server-dependency-licenses.json s3://chef-automate-artifacts/dev/latest/chef-server/licenses.json --acl public-read --profile chef-cd
...
aws s3 cp "s3://chef-automate-artifacts/dev/latest/chef-server/versions.json" existing-versions.json --profile chef-cd || echo "[]" > existing-versions.json
...
aws s3 cp "$VERSION.json" s3://chef-automate-artifacts/dev/latest/chef-server/manifest.json --acl public-read --profile chef-cd
aws s3 cp updated-versions.json s3://chef-automate-artifacts/dev/latest/chef-server/versions.json --acl public-read --profile chef-cd
